...
class SafeController:
    ...
    async def move_cursor(self, direction: str, distance: int = 100):
        ...
        return f"Mouse cursor moved {direction}."
    ...
    async def mouse_click(self, button: str = "left"):
        ...
        return f"{button.capitalize()} click performed."
    ...
    async def type_text(self, text: str):
        ...
        return f"Text typed successfully: {text}"
    ...
    async def press_hotkey(self, keys: List[str]):
        ...
        for k in keys:
            if k.lower() not in self.special_keys and k.lower() not in self.valid_keys:
                return f"Invalid key in hotkey combination: {k}"
        ...
        return f"Hotkey {' + '.join(keys)} pressed successfully."
    ...
    async def control_volume(self, action: str):
        ...
        return f"Volume {action} applied."
    ...
    async def swipe_gesture(self, direction: str):
        ...
        return f"Swipe {direction} performed."
...
async def with_temporary_activation(fn, *args, **kwargs):
    """Temporarily activates the controller, executes the function, and deactivates it."""
...
async def mouse_click_tool(button: str = "left"):
    ...
    Simulates clicking behaviour for automation or voice command triggers.
...
async def scroll_cursor_tool(direction: str, amount: int = 10):
    ...
    Useful for commands such as "scroll down" or "scroll up".
...
async def type_text_tool(text: str):
    ...
    Useful for commands such as "type hello world" or "write the following text".
...
async def press_key_tool(key: str):
    """
    Simulates pressing a single key on the keyboard, such as Enter, Escape, or any letter/number.

    Useful for commands such as "press Enter", "press Escape", or "press the A key".
...
async def press_hotkey_tool(keys: List[str]):
    """
    Simulates pressing a keyboard shortcut such as Ctrl+S, Alt+F4, etc.

    Use this when the user requests actions such as "save the file",
    "close the window", or "refresh the page".
...
async def control_volume_tool(action: str):
    ...
    Use this when the user requests actions such as "increase the volume",
    "mute the sound", or "lower the volume".
...
async def swipe_gesture_tool(direction: str):
    ...
    Use this when the user wishes to swipe in a direction such as up, down,
    left, or right — for example: "swipe left", "swipe down", or "scroll the screen up".
